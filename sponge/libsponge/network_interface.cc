...
void NetworkInterface::send_datagram(const InternetDatagram &dgram, const Address &next_hop) {
    ...
    const auto &arp_iter = _arp_table.find(next_hop_ip); // search the ARP table
    if (arp_iter == _arp_table.end()) {
        ...
        // generate ethernetframe
        EthernetFrame eth_frame;
        eth_frame.header() = {arp_iter->second.eth_addr, _ethernet_address, EthernetHeader::TYPE_IPv4};
